...
class FlxTypedGroup<T:FlxBasic> extends FlxBasic
{
	...
	public function add(Object:T):T
	{
		...
		if (index != -1)
		{
			...
			if (_memberAdded != null) _memberAdded.dispatch(Object);

			return Object;
		}

		// If the group is full, return the Object
		if (maxSize > 0 && length >= maxSize) return Object;
	...
	inline function recycleCreateObject(?ObjectClass:Class<T>, ?ObjectFactory:Void->T):T
	{
		...
		if (ObjectFactory != null) add(object = ObjectFactory());
		else if (ObjectClass != null) add(object = Type.createInstance(ObjectClass, []));
	...
	public function replace(OldObject:T, NewObject:T):T
	{
		...
		if (_memberRemoved != null) _memberRemoved.dispatch(OldObject);
		if (_memberAdded != null) _memberAdded.dispatch(NewObject);
	...
	public function getFirstExisting():T
	{
		...
		while (i < length)
		{
			...
			if (basic != null && basic.exists) return cast basic;
		}
	...
	public function getFirstAlive():T
	{
		...
		while (i < length)
		{
			...
			if (basic != null && basic.exists && basic.alive) return cast basic;
		}
	...
	public function getFirstDead():T
	{
		...
		while (i < length)
		{
			...
			if (basic != null && !basic.alive) return cast basic;
		}
	...
	public function clear():Void
	{
		...
		if (_memberRemoved != null)
		{
			...
			{
				if (member != null) _memberRemoved.dispatch(member);
			}
		}
		...
		while (i < length)
		{
			...
			if (basic != null)
			{
				if (Recurse)
				{
					...
					if (group != null) group.forEach(cast Function, Recurse);
				}
	...
	public function forEachAlive(Function:T->Void, Recurse:Bool = false)
	{
		...
		while (i < length)
		{
			...
			if (basic != null && basic.exists && basic.alive)
			{
				...
				{
					var group = resolveGroup(basic);
					if (group != null) group.forEachAlive(cast Function, Recurse);
				}
	...
	public function forEachDead(Function:T->Void, Recurse:Bool = false)
	{
		...
		while (i < length)
		{
			...
			if (basic != null && !basic.alive)
			{
				...
				{
					var group = resolveGroup(basic);
					if (group != null) group.forEachDead(cast Function, Recurse);
				}
	...
	public function forEachExists(Function:T->Void, Recurse:Bool = false)
	{
		...
		while (i < length)
		{
			...
			if (basic != null && basic.exists)
			{
				...
				{
					var group = resolveGroup(basic);
					if (group != null) group.forEachExists(cast Function, Recurse);
				}
	...
	public function forEachOfType<K>(ObjectClass:Class<K>, Function:K->Void, Recurse:Bool = false)
	{
		...
		while (i < length)
		{
			...
			if (basic != null)
			{
				...
				{
					var group = resolveGroup(basic);
					if (group != null) group.forEachOfType(ObjectClass, cast Function, Recurse);
				}

				if (isOfType(basic, ObjectClass)) Function(cast basic);
			}
		}
	}
	...
	function set_maxSize(Size:Int):Int
	{
		...
		if (maxSize == 0 || members == null || maxSize >= length) return maxSize;
		...
		while (i < l)
		{
			...
			if (basic != null)
			{
				if (_memberRemoved != null) _memberRemoved.dispatch(cast basic);
