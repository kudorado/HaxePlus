...
class FlxG
{
	...
	/**
	 * Used to add messages to the log window or enable `//KUDORADOtrace()` redirection.
	 */
	...
	{
		if (state.switchTo(nextState)) game._requestedState = nextState;
	}
	...
	public static inline function openURL(URL:String, Target:String = "_blank"):Void
	{
		...
		if (!~/^https?:\/\//.match(URL)) prefix = "http://";
	...
	{
		if (Framerate < drawFramerate) log.warn("FlxG.framerate: the game's framerate shouldn't be smaller than the flash framerate,"
		...
		if (game._maxAccumulation < game._stepMS) game._maxAccumulation = game._stepMS;
	...
	{
		if (Framerate > updateFramerate) log.warn("FlxG.drawFramerate: the update framerate shouldn't be smaller than the draw framerate,"
			+ " since it can stop your game from updating.");

		drawFramerate = Std.int(Math.abs(Framerate));

		if (game.stage != null) game.stage.frameRate = drawFramerate;
