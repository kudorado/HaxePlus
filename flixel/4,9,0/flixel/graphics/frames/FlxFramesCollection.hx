...
class FlxFramesCollection implements IFlxDestroyable
{
	...
	public function new(parent:FlxGraphic, ?type:FlxFrameCollectionType, ?border:FlxPoint)
	{
		...
		if (parent != null) parent.addFrameCollection(this);
	}
	...
	{
		if (name != null && framesHash.exists(name)) return framesHash.get(name);
	...
	function checkFrame(frame:FlxRect, ?name:String):FlxRect
	{
		...
		if (frame.width <= 0 || frame.height <= 0) FlxG.log.warn("The frame " + name + " has incorrect data and results in an image with the size of (0, 0)");
	...
	public function pushFrame(frameObj:FlxFrame):FlxFrame
	{
		...
		if (name != null) framesHash.set(name, frameObj);
