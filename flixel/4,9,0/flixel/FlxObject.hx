...
class FlxObject extends FlxBasic
{
	...
	{
		if (axes == null) axes = FlxAxes.XY;

		if (axes != FlxAxes.Y) x = (FlxG.width / 2) - (width / 2);
		if (axes != FlxAxes.X) y = (FlxG.height / 2) - (height / 2);
	...
	{
		if (FlxG.renderBlit) camera.buffer.draw(FlxSpriteUtil.flashGfxSprite);
	}
	#end
	...
	function set_path(path:FlxPath):FlxPath
	{
		...
		if (this.path != null) this.path.object = null;

		if (path != null) path.object = this;
