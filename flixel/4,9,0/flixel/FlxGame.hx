...
class FlxGame extends Sprite
{
	...
	function create(_):Void
	{
		...
		#if FLX_POST_PROCESS
		if (OpenGLView.isSupported) addChild(postProcessLayer);
		#end
	...
	{
		#if next
		if (event != null && event.target != FlxG.stage) return;
		#end
	...
	inline function resetGame():Void
	{
		...
		if (_skipSplash || FlxSplash.nextState != null) // already played
		{
			...
			if (FlxSplash.nextState == null) _gameJustStarted = true;
		}
		else
		{
		...
		#if FLX_DEBUG
		if ((_requestedState is FlxSubState)) throw "You can't set FlxSubState class instance as the state for you game";
		#end
