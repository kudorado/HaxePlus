...
class Console extends Window
{
	...
	function registerEventListeners()
	{
		...
		{
			if (FlxG.debugger.visible && FlxG.game.debugger.console.visible && e.keyCode == Keyboard.TAB) FlxG.stage.focus = input;
		});
		#end
		#end
	...
	function onFocus(_)
	{
		#if FLX_DEBUG
		// Pause game
		if (FlxG.console.autoPause) FlxG.vcr.pause();
		...
		if (input.text == Console.DEFAULT_TEXT) input.text = "";
		#end
	}
	...
	function onFocusLost(_)
	{
		#if FLX_DEBUG
		// Unpause game
		if (FlxG.console.autoPause && !FlxG.game.debugger.vcr.manualPause) FlxG.vcr.resume();
		...
		if (input.text == "") input.text = Console.DEFAULT_TEXT;
		#end
	...
	function onKeyDown(e:KeyboardEvent)
	{
		...
		switch (e.keyCode)
		{
			...
			case Keyboard.UP:
				if (!history.isEmpty) setText(history.getPreviousCommand());

			case Keyboard.DOWN:
				if (!history.isEmpty) setText(history.getNextCommand());
		}
	}
	...
	function processCommand()
	{
		try
		{
			...
			if (registeredFunctions.get(text) != null) text += "()";
			...
			#if FLX_DEBUG
			if (FlxG.vcr.paused && FlxG.console.stepAfterCommand) FlxG.game.debugger.vcr.onStep();
			#end
	...
	public function registerFunction(functionAlias:String, func:Dynamic, ?helpText:String)
	{
		...
		if (helpText != null) registeredHelp.set(functionAlias, helpText);
	}
