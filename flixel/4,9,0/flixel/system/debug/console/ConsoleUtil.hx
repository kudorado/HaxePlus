...
class ConsoleUtil
{
	...
	{
		if (AnyObject == null || Reflect.isObject(AnyObject)) interp.variables.set(ObjectAlias, AnyObject);
	}
	...
	{
		if (Reflect.isFunction(Function)) interp.variables.set(FunctionAlias, Function);
	}
	#end
	...
	public static function getFields(Object:Dynamic):Array<String>
	{
		...
		for (field in fields)
		{
			...
			if (field.startsWith("get_") || field.startsWith("set_"))
			{
				...
				if (!fields.contains(name) && !filteredFields.contains(name)) filteredFields.push(name);
			}
			else
